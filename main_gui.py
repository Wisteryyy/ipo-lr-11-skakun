...
class TransportCompanyApp: # класс TransportCompanyApp, который представляет приложение транспортной компании
    def __init__(self, root): # метод инициализации класса, принимающий главное окно
        ...
        self.clients_data = self.load_data_from_json() # загружаем данные клиентов из JSON-файла
    ...
    def load_data_from_json(self): # метод для загрузки данных клиентов из JSON-файла
        try:
    ...
    def save_data_to_json(self, data): # метод для сохранения данных клиентов в JSON-файл
    ...
    def delete_client(self): # метод для удаления выбранного клиента
        ...
        self.save_data_to_json(self.company.clients) # сохраняем обновленный список клиентов в JSON файл
